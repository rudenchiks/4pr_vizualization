...
let createSortArr = (data) => {
    let sortArr = [];
    let usedColumns = [];  // Массив для отслеживания использованных колонок
    ...
    for (let i = 0; i < sortSelects.length; i++) {
        ...
        // Если выбрано "Нет", завершаем формирование массива
        if (keySort == 0) {
            break;
        }

        // Проверяем, был ли уже использован этот столбец
        if (usedColumns.includes(keySort)) {
            continue; // Пропускаем этот столбец, если он уже был выбран на предыдущем уровне
        }

        // Добавляем столбец в массив использованных
        usedColumns.push(keySort);
...
let sortTable = (idTable, data) => {
    ...
    rowData.sort((first, second) => {
        for (let i in sortArr) {
            ...
            // Для столбцов "Год" и "Рейтинг" преобразуем значения в числа
            if (columnNames[key] === "Год" || columnNames[key] === "Рейтинг") {
...
let resetSort = (idTable, dataForm) => {
    ...
    if (filterForm) {
        ...
        filterTable(films, idTable, filterForm);
    } else {
        // Иначе просто создаем таблицу с исходными данными
        createTable(films, idTable);
    }
};
